...
const TotalIngredients = ({isTotalOpen, setIsTotalOpen}: {isTotalOpen: boolean; setIsTotalOpen: any}) => {
  const weekState = useAppSelector((state) => state.app)

  const ingredientsList = weekState.map(item => {
    let ingredients: any[] = [];
  ...
  }).flat().filter((ing: any) => ing.checked).sort((a: any, b: any)  => a.type > b.type ? -1 : 1 )

  let addedQuantity = ingredientsList.map((item: any, index) => {
    let possibleDuplicates = ingredientsList.filter((ing: any) => ing.name === item.name && ing.unit === item.unit);
